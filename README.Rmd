...
The library `SignalCancellation` offers the `scrof`, `SCROF`, or `SCRoF` function for Signal cancellation recovery of factors [@Achim24].
...
We can also try the @TB19 correlation matrix directly.
...
The saturation and correlation between factors are found here (from `test_k_dim`).
...
There is a function to compute the correlation matrix by excluding factors.
...
Inspection can be done with the following function. It yields the residual covariances, their *p*-values and partial correlations.
...
Caron, P.-O. & Achim, A. (2025). *SignalCancellation*. https://github.com/quantmeth/SignalCancellation
